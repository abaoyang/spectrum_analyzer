 ...
 * 描述    ：LCD驱动代码,支持驱动IC型号包括:ILI9341/ILI9325/RM68042/RM68021/ILI9320/ILI9328/LGDP4531/LGDP4535/SPFD5408/SSD1289/1505/B505/C505等  
 *
...
/*
 * 函数名：LCD_BGR2RGB
 * 描述  ：从LCD读出的数据为GBR格式，而我们写入的时候为RGB格式，通过该函数转换。
 * 输入  ：c:GBR格式的颜色值
 * 输出  ：返回值：RGB格式的颜色值
 */
...
/*
 * 函数名：LCD_Scan_Dir
 * 描述  ：注意:其他函数可能会受到此函数设置的影响(尤其是9341/6804这两个奇葩),
...
/*
 * 函数名：LCD_Init
 * 描述  ：初始化lcd，该初始化函数可以初始化各种ILI93XX液晶,但是其他函数是基于ILI9320的
					 在其他部分型号的驱动芯片上没有测试!
 * 输入  ：dir:0,竖屏；1,横屏
 * 输出  ：无
 */
...
//GPIO_ResetBits(GPIOB, GPIO_Pin_6);	 								//低电平LCD复位
//GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable,ENABLE);	//SWD ENABLE,JTAG Disable PB3,4 -GPIO
...
//   GPIO_ResetBits(GPIOB, GPIO_Pin_6);	 //低电平复位
// 	delay_ms(50); 					   
//   GPIO_SetBits(GPIOB, GPIO_Pin_6 );		 	  
  ...
  if(lcddev.id<0XFF||lcddev.id==0XFFFF||lcddev.id==0X9300)//读到ID不正确,新增lcddev.id==0X9300判断，因为9341在未被复位的情况下会被读成9300
	{	
		...
		if(lcddev.id!=0X9341&&lcddev.id!=0X6804)lcddev.id=0x9325;//新增，用于识别9341 	     
		}
 	printf(" LCD ID is:%x\r\n",lcddev.id); //打印LCD ID  
 ...
 * 函数名：LCD_ShowNum
 * 描述  ：显示数字,高位为0,还是显示
 * 输入  ：x,y:起点坐标，num:数值(0~999999999)，len :数字的位数，size:字体大小
 * 输出  ：无
 */ 
 ...
 * 函数名：LCD_ShowxNum
 * 描述  ：显示数字,高位为0,还是显示
 * 输入  ：x,y:起点坐标，num:数值(0~999999999)，len:长度(即要显示的位数)，size:字体大小，
					mode:
					[7]:0,不填充;1,填充0.
					[6:1]:保留
					[0]:0,非叠加显示;1,叠加显示.
 ...
 * 函数名：LCD_ShowString
 * 描述  ：显示字符串
 * 输入  ：x,y:起点坐标，width,height:区域大小，size:字体大小，*p:字符串起始地址		 
 * 输出  ：无
 */	  
